...
export function formatearBusqueda(
  ...
  if (resultado.bbva.length > 0) {
    ...
    resultado.bbva.slice(0, LIMITE).forEach((item, index) => {
      mensaje += `${index + 1}.\n`;
  ...
  if (resultado.agrario.length > 0) {
    ...
    const LIMITE = 10;
    resultado.agrario.slice(0, LIMITE).forEach((item, index) => {
      mensaje += `${index + 1}.\n`;
      mensaje += `Código: ${item.codigo_convenio}\n`;
      mensaje += `Convenio: ${item.nombre_convenio}\n`;
      mensaje += `NIT: ${item.nit}\n\n`;
  ...
  if (resultado.aval.length > 0) {
    ...
    resultado.aval.slice(0, LIMITE).forEach((item, index) => {
      mensaje += `${index + 1}.\n`;
  ...
  if (resultado.bbva.length > LIMITE) {
    mensaje += `... y ${resultado.bbva.length - LIMITE} más.\n\n`;
  }
