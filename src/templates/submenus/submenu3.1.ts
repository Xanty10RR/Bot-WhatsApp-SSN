...
export const RequisicionSolicitud = addKeyword(MENU_IDS.SUBMENU_3.OPCION1)
    .addAnswer("¡Vamos a crear una nueva requisición!.\nPor favor, dime tu *nombre completo*:", { capture: true }, async (ctx, { state }) => {
        await state.update({ nombre: ctx.body });
    })
    .addAnswer("Por favor coloca tu *número de cédula* completo:", { capture: true }, async (ctx, { state }) => {
    ...
    .addAnswer(
        ...
        async (ctx, { flowDynamic, state }) => {
            const mapaDept: Record<string, string> = { '1': 'Logística', '2': 'RRHH', '3': 'IT/Sistemas', '4': 'Comercial', '5': 'Otros' };
    ...
    .addAnswer("⚠️ Si es Asesor escriba el *código de punto de venta* o si es *administrativo marque 0000*:", { capture: true }, async (ctx, { flowDynamic, state }) => {
        const input = ctx.body.trim();
        if (!/^\d+$/.test(input)) {
            return await flowDynamic("❌ Formato inválido. Debes ingresar solo números.");
        }
    ...
    .addAnswer(
        ...
        async (ctx, { flowDynamic, state }) => {
            ...
            const idx = parseInt(ctx.body.trim()) - 1;
            if (idx < 0 || idx > 3) return await flowDynamic("❌ Opción inválida.");
            await state.update({ tipo_solicitud: tipos[idx] });
        }
    )
    .addAnswer(
        ...
        async (ctx, { flowDynamic, state }) => {
            ...
            const idx = parseInt(ctx.body.trim()) - 1;
            if (idx < 0 || idx > 3) return await flowDynamic("❌ Opción inválida.");
            await state.update({ tipo_elemento: tipos[idx] });
        }
    )
    ...
    .addAnswer("Indique la cantidad necesaria:", { capture: true }, async (ctx, { flowDynamic, state }) => {
    ...
    .addAction(async (ctx, { state, flowDynamic }) => {
        ...
        await pool.query(
            ...
            [
                ctx.from, datos.nombre, datos.cedula, datos.departamento_destino, 
                datos.asesor_o_administrativo, datos.tipo_solicitud, 
                datos.tipo_elemento, datos.descripcion, datos.cantidad, datos.observaciones
            ]
        );
        ...
        await flowDynamic([{ body: '¿Qué deseas hacer ahora?', buttons: [{ body: '📝 Nueva requisición' }, { body: '🏠 Menú principal' }] }]);
    })
    .addAction({ capture: true }, async (ctx, { gotoFlow }) => {
        if (ctx.body.includes('Nueva') || ctx.body.includes('📝')) return gotoFlow(RequisicionSolicitud);
